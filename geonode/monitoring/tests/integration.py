...
class TestClient(DjangoTestClient):
    def _base_environ(self, **request):
        ...
        environ = {
            "HTTP_COOKIE": "; ".join(sorted(f"{morsel.key}={morsel.coded_value}" for morsel in self.cookies.values())),
            "PATH_INFO": "/",
...
class RequestsTestCase(MonitoringTestBase):
    def setUp(self):
        ...
        self.u.save()
        self.ua = (
            "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36" "(KHTML, like Gecko) Chrome/59.0.3071.47 Safari/537.36"
        )
        populate()
        ...
        self.service, _ = Service.objects.get_or_create(
            name=settings.MONITORING_SERVICE_NAME, host=self.host, service_type=self.service_type
        )
    ...
    def test_gn_request(self):
        ...
        self.client.get(reverse("dataset_embed", args=(_l.alternate,)), **{"HTTP_USER_AGENT": self.ua})

        if RequestEvent.objects.all().count():
            rq = RequestEvent.objects.all().last()
            self.assertTrue(rq.response_time > 0)
            self.assertEqual(
                list(rq.resources.values_list("name", "type")),
                [
                    (
                        _l.alternate,
                        "dataset",
    ...
    def test_gn_error(self):
        ...
        self.client.get(reverse("dataset_embed", args=("nonex",)), **{"HTTP_USER_AGENT": self.ua})
    ...
    def test_service_handlers(self):
        ...
        if requests:
            ...
            c.process_requests(self.service, requests, q.first().created, q.last().created)
            ...
            # Works only with Postgres
            metrics = c.get_metrics_for(
                metric_name="request.ip", valid_from=valid_from, valid_to=valid_to, interval=interval
            )
...
class MonitoringUtilsTestCase(MonitoringTestBase):
    ...
    def test_time_periods(self):
        ...
        # 2017-06-20 12:22:50
        start = datetime(year=2017, month=0o6, day=20, hour=12, minute=22, second=50)
        # 2017-06-20 12:20:00
        start_aligned = datetime(year=2017, month=0o6, day=20, hour=12, minute=20, second=0)
...
class MonitoringChecksTestCase(MonitoringTestBase):
    ...
    def test_monitoring_checks(self):
        ...
        resource, _ = MonitoredResource.objects.get_or_create(type="dataset", name="test:test")
        resource2, _ = MonitoredResource.objects.get_or_create(type="dataset", name="test:test2")

        label, _ = MetricLabel.objects.get_or_create(name="discount")
        MetricValue.add(
            self.metric, start_aligned, end_aligned, self.service, label="Count", value_raw=10, value_num=10, value=10
        )
        uthreshold = [
        ...
        mc, _ = MetricNotificationCheck.objects.get_or_create(
    ...
    def test_notifications_views(self):
        ...
        nc, _ = NotificationCheck.objects.get_or_create(name="check requests", description="check requests")

        MetricNotificationCheck.objects.get_or_create(
            notification_check=nc,
            service=self.service,
            metric=self.metric,
            min_value=10,
        ...
        nresp = self.client.get(reverse("monitoring:api_user_notification_config", kwargs={"pk": nc.id}))
        self.assertIsNotNone(nresp)
        self.assertEqual(nresp.status_code, 200, nresp)
        data = json.loads(ensure_string(nresp.content))
    ...
    def test_notifications_api(self):
        ...
        for nc in NotificationCheck.objects.all():
            ...
            if resp.status_code == 200:
                ...
                self.assertEqual({u.email for u in nc.get_users()}, set(_users))
                self.assertEqual({email for email in nc.get_emails()}, set(_emails))

        metric_rq_count = Metric.objects.get(name="request.count")
        metric_rq_time = Metric.objects.get(name="response.time")

        MetricValue.add(
            metric_rq_count, start_aligned, end_aligned, self.service, label="Count", value_raw=0, value_num=0, value=0
        )
        ...
        self.assertEqual({n["id"] for n in ndata["data"]}, set(NotificationCheck.objects.values_list("id", flat=True)))
        mail.outbox = []
        self.assertEqual(len(mail.outbox), 0)
        capi.emit_notifications(start)
        self.assertEqual(len(mail.outbox), 0)
...
class MonitoringAnalyticsTestCase(MonitoringTestBase):
        ...
        users_count_data = [
            {
                ...
                "metric_count": 16,
                "min": "1.0000",
                "resource": {"href": "/", "id": 1, "name": "/", "type": "url"},
                "samples_count": 17,
                "sum": "17.0000",
                "val": 5,
            },
            {
                ...
                "metric_count": 3,
                "min": "1.0000",
                "resource": {"href": "", "id": 5, "name": "geonode:waterways", "type": "layer"},
                "samples_count": 5,
                "sum": "5.0000",
                "val": 2,
            },
            {
                ...
                "metric_count": 2,
                "min": "1.0000",
                "resource": {"href": "", "id": 2, "name": "geonode:roads", "type": "layer"},
                "samples_count": 3,
                "sum": "3.0000",
                "val": 1,
            },
            {
                ...
                "metric_count": 2,
                "min": "1.0000",
                "resource": {"href": "", "id": 3, "name": "geonode:railways", "type": "layer"},
                "samples_count": 4,
                "sum": "4.0000",
                "val": 1,
            },
            {
                ...
                "metric_count": 1,
                "min": "1.0000",
                "resource": {"href": "", "id": 4, "name": "San Francisco Transport Map", "type": "map"},
                "samples_count": 1,
                "sum": "1.0000",
                "val": 1,
            },
            {
                ...
                "metric_count": 1,
                "min": "1.0000",
                "resource": {"href": "", "id": 6, "name": "Amsterdam Waterways Map", "type": "map"},
                "samples_count": 1,
                "sum": "1.0000",
                "val": 1,
            },
    ...
    def test_resources_count_endpoint(self):
        resource_count_data = [
            {"max": "4.0000", "metric_count": 16, "min": "1.0000", "samples_count": 31, "sum": "31.0000", "val": 6}
        ]
        # url
        url = (
            f"{reverse('monitoring:api_metric_data', args={'request.count'})}?"
    ...
    def test_unique_visitors_count_endpoints(self):
        unique_visitors_data = [
            {"max": "3.0000", "metric_count": 25, "min": "1.0000", "samples_count": 31, "sum": "31.0000", "val": 5}
        ]
        # url
        ...
        for d in data:
            ...
            is_empty = [
                ...
                if not (md["max"] or md["metric_count"] or md["min"] or md["samples_count"] or md["sum"] or md["val"])
            ]
            if is_empty:
                empty_months += 1
            else:
    ...
    def test_anonymous_sessions_count_endpoints(self):
        session_data = [
            {"max": "1.0000", "metric_count": 7, "min": "1.0000", "samples_count": 7, "sum": "7.0000", "val": 7}
        ]
        # url
        url = (
            f"{reverse('monitoring:api_metric_data', args={'request.users'})}?"
    ...
    def test_hostgeonode_cpu_endpoints(self):
        ...
        url = (
            f"{reverse('monitoring:api_metric_data', args={'cpu.usage.percent'})}?"
    ...
    def test_hostgeonode_mem_endpoints(self):
        ...
        url = (
            f"{reverse('monitoring:api_metric_data', args={'mem.usage.percent'})}?"
            f"{'valid_from=2018-09-11T20:00:00.000Z&valid_to=2019-09-11T20:00:00.000Z&interval=31536000'}&{'service=localhost-hostgeonode'}"
        )
        response = self.client.get(url)
        out = json.loads(ensure_string(response.content))
    ...
    def test_countries_endpoint(self):
        # url
        url = (
            f"{reverse('monitoring:api_metric_data', args={'request.country'})}?"
